...
def set_json_argument(new_args_dict: Dict[str, Any]):
    ...
    with filelock.FileLock(os.path.join(server_properties_dir, 'server_properties.json.lock')):  # Acquire a lock to prevent race conditions
        with open(os.path.join(server_properties_dir, 'server_properties.json'), 'r+') as f:  # Open file for reading and writing
            try:
